...
class PSENet(LightningModule): #нейронка для ??? сегментации текста на изображениях
    ...
    def configure_optimizers(self): # переопределения функции родителя
        optimizer = torch.optim.Adam(self.model.parameters(), lr=self.lr, betas=[0.9, 0.99]) # это то что обучает нейронку и меняет в ней веса
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=10, factor=0.5, verbose=True) #динамически изменять скорость обучения в зависимости от поведения функции потерь. Если в течение определенного количества эпох (указано параметром patience) значение функции потерь не уменьшится, скорость обучения будет уменьшена в factor раз.
    ...
    def training_epoch_end(self, outputs): # переопределения функции родителя . вызывается в конце каждой
        ...
        if isinstance(sch, torch.optim.lr_scheduler.ReduceLROnPlateau):
            sch.step(self.trainer.callback_metrics["total_loss"]) ## адаптирует шаг обучения в зависимости от функции потерь

    def generate_pseudo_gt(self, im): #генерирует псевдо-целевое изображение
        bs, ch, h, w = im.shape  #Получает размерности входного изображения im - размер пакета (bs), количество каналов (ch), высоту (h) и ширину (w).
        ...
        overrexposed_ranges = torch.linspace(self.gamma_lower, 0, steps=self.number_refs + 1).to(im.device)[:-1] #  Создает значения гамма для недо- и переэкспонированных областей
        ...
        synthetic_references = 1 - (1 - im[:, None]) ** gammas[:, :, None, None, None] ##Синтез синтетических образцов с разными значениями гамма:
        # Генерирует синтетические образцы с различными значениями гамма,
        # используя значения гамма для недо- и переэкспонированных областей.
        ...
        scores = self.iqa(references.view(bs * nref, ch, h, w)) # Вычисление оценок качества изображений: Оценивает качество каждого синтетического образца
        scores = scores.view(bs, nref, 1, h, w) #Собирает псевдо-целевое изображение, объединяя входное изображение, предыдущий вывод и выбранный лучший синтетический образец
    ...
    def validation_step(self, batch, batch_idx): #Метод validation_step используется для
        ...
        if batch_idx == 0:
            pred_im, pred_gamma = self.model(batch)  #Модель используется для получения предсказанных значений на текущем пакете данных.
